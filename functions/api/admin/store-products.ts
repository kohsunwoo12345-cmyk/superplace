...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // 실제 프로덕션 스키마 순서에 맞춰서 INSERT (33개 컬럼, dailyChatLimit은 맨 마지막)
    await env.DB.prepare(`
...
/**
 * PUT /api/admin/store-products?id=xxx
 * 제품 수정
 */
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (!isAdmin(tokenData.role)) {
      return new Response(JSON.stringify({ 
        error: 'Insufficient permissions',
        message: `Only ADMIN or SUPER_ADMIN can update products. Your role: ${tokenData.role}` 
      }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' }
      });
...
/**
 * DELETE /api/admin/store-products?id=xxx
 * 제품 삭제
 */
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (!isAdmin(tokenData.role)) {
      return new Response(JSON.stringify({ 
        error: 'Insufficient permissions',
        message: `Only ADMIN or SUPER_ADMIN can delete products. Your role: ${tokenData.role}` 
      }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' }
      });
